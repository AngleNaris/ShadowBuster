...
- 1.5.0 最终构建：CPU runtime 从空目录重建（`-Flavor cpu`，torch/torchaudio 2.7.1+cpu，DSP 脚本与 critical-manifest 校验通过），PyInstaller 外壳 + Inno Setup 编译通过。安装器 `ShadowBuster-Setup-1.5.0.exe` 大小 `788014319` bytes，SHA-256：`76ea6a1b6fcf5a825a854230a9129c5af6303ef7bcb49dcb2082b3fb980af15c`。
- 1.5.0 GPU 环境包：`build_gpu_env.ps1` 产出 `gpu-env-1.5.0.zip`（SHA-256 `4e1afcf66adcfb3868b25199f88aac72389a111d3bcc8689f4cc8c45bf410338`，3,603,190,880 bytes）+ 3 个分卷 + `gpu-env-1.5.0.json` 清单，全部上传 v1.5.0 Release；分卷经 `gpu_env.assemble_zip` 本地端到端验证（组装大小与整包哈希均匹配）；应用「检查更新/GPU 环境」经公开仓库匿名 API 可用。
- 测试：164 passed, 13 subtests passed（含新增 `tests/test_gpu_env.py` 32 项：清单校验/续传/组装/哈希/解压/切换，无网络依赖）。
