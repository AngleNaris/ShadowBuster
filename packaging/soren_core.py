...
def process_original_styled(target, reference, step, config, genre_profile):
    ...
    if not np.isfinite(audio).all():
        raise ValueError("Soren input contains non-finite samples")
    peak = float(np.max(np.abs(audio)))
    if peak == 0:
        raise ValueError("Cannot master silent audio")
...
def process_audio(target, reference, step, config, genre_profile=None):
    if config.style_mode == "styled":
        return process_original_styled(target, reference, step, config, genre_profile)
...
def master_audio(input_file, output_file, config, eq_style, is_preview=False):
    ...
    output_ceiling_db = -0.4 if config.style_mode == "styled" else config.true_peak_ceiling_db
    if written_true_peak_db > output_ceiling_db + 1e-5:
        raise RuntimeError(
            f"PCM24 output exceeds True Peak ceiling: "
            f"{written_true_peak_db:.6f} > {output_ceiling_db:.6f} dBTP"
        )
