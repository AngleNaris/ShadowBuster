...
面向 AI 生成音乐（Suno 等）与有损音频的一键式修复 + 母带桌面应用。信号链五阶段：

```
Lew 高频重建 → Demucs 四轨分离 → 贝斯增强 → 鼓增强 → Soren 母带
```
...
| 目录 | 内容 | 来源 |
|---|---|---|
| `Apollo/` | 上游 [JusperLee/Apollo](https://github.com/JusperLee/Apollo) 源码（`look2hear/` 等）+ 本项目管线脚本 + `ckpts/` Lew 权重 | 上游仓库 + 权重另行获取 |
| `Soren_src/` | Soren 母带链：`core_decrypted.py`、`model/`、`profiles/`、`secured_genres/` | 闭源组件，自行持有 |
| ffmpeg | `ffmpeg.exe` 加入 PATH | 官方构建 |

`Apollo/` 根目录除上游源码外，还需放入本项目维护的管线脚本：
`lew_upscale.py`（高频重建入口）、`bass_enhance.py`（贝斯增强）、`drum_enhance.py`（鼓增强）、`low_punch.py`。
